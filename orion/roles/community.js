...
//======================================================//
/** 
 * Users can see contributors in the admin
 */
CommunityRole.allow('collections.contributors.index', true);

/**
 * And we will make that the users only see their contributors in the index
 */
CommunityRole.helper('collections.contributors.indexFilter', function() {
  return { createdBy: this.userId };
})

/**
 * Users can create contributors
 */
CommunityRole.allow('collections.contributors.insert', true);

/**
 * Users can update contributors
 */
CommunityRole.allow('collections.contributors.update', function(userId, doc, fields, modifier) {
  return doc.createdBy === userId; // Will be allowed to edit his own contributors
});

/**
 * Users can't change the createdBy attribute
 */
CommunityRole.deny('collections.contributors.update', function(userId, doc, fields, modifier) {
  return !_.contains(fields, 'userId');
});

/**
 * Users can remove contributors
 */
CommunityRole.allow('collections.contributors.remove', function(userId, doc) {
  return doc.createdBy === userId; // Will be allowed to edit his own contributors
});

/**
 * Users can see the create contributor button
 */
CommunityRole.allow('collections.contributors.showCreate', true);

/**
 * Users can see the update contributor button if they created the doc
 */
CommunityRole.allow('collections.contributors.showUpdate', function(doc) {
  return doc.createdBy == this.userId;
});

/**
 * Users can see the delete contributor button if they created the doc
 */
CommunityRole.allow('collections.contributors.showRemove', function(doc) {
  return doc.createdBy == this.userId;
});
